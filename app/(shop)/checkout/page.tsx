...
import {
  ShoppingBag, Truck, CheckCircle2, Loader2, ArrowLeft, MapPin, User, Trash2,
} from "lucide-react";
...
export default function CheckoutPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const shippingPerItem = items.length > 0
      ? Math.round((shippingCost / items.length) * 100) / 100
      : 0;
      ...
      for (const item of items) {
        const res = await fetch("/api/checkout", {
          ...
          body: JSON.stringify({
            ...
            total_price: Math.round((item.product.price * item.quantity + shippingPerItem) * 100) / 100,
          }),
        });
      ...
      <div className="min-h-screen bg-background flex items-center justify-center px-4 pt-20">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link
              href="/track"
              className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-2xl font-bold text-sm border border-border hover:bg-accent transition-colors"
            >
              Track Order
            </Link>
            <Link
              href="/shop"
              className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-2xl font-bold text-sm"
            ...
            <div className="lg:col-span-7">
              <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      <div className="grid sm:grid-cols-2 gap-4">
                        ...
                        >
                          <div className="flex justify-between items-center mb-1">
                            <span className={cn("text-sm font-bold", form.shipping_area === "inside" ? "text-amber-700" : "text-foreground")}>
                              INSIDE DHAKA
                            </span>
                            {form.shipping_area === "inside" && <div className="size-2 rounded-full bg-amber-500" />}
                          </div>
                          <span className="text-xs font-bold text-muted-foreground">৳{shippingRates.inside} FLAT RATE</span>
                        </button>
                        ...
                        >
                          <div className="flex justify-between items-center mb-1">
                            <span className={cn("text-sm font-bold", form.shipping_area === "outside" ? "text-amber-700" : "text-foreground")}>
                              OUTSIDE DHAKA
                            </span>
                            {form.shipping_area === "outside" && <div className="size-2 rounded-full bg-amber-500" />}
                          </div>
                          <span className="text-xs font-bold text-muted-foreground">৳{shippingRates.outside} FLAT RATE</span>
                        </button>
                      </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
                        ...
                        Delivery: 1-2 days (Inside Dhaka), 3-5 days (Outside).
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  style={{ background: "oklch(0.78 0.18 72)", color: "oklch(0.09 0 0)" }}
                >
                  {loading ? <Loader2 className="size-5 animate-spin" /> : <>CONFIRM ORDER · ৳{total.toLocaleString()}</>}
                </button>
              </form>
            </div>
              ...
              <div className="rounded-3xl border border-border bg-card overflow-hidden sticky top-24 shadow-sm">
                      ...
                      <div className="flex-1 min-w-0 flex flex-col">
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            <p className="text-sm font-bold text-foreground truncate uppercase tracking-tight">{item.product.name}</p>
                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide mt-0.5">
                              SIZE: {item.size} · QTY: {item.quantity}{item.color?.name ? ` · ${item.color.name}` : ""}
                            </p>
                          </div>
                          <button
                            type="button"
                        ...
                        <p className="text-sm font-bold mt-auto">৳{(item.product.price * item.quantity).toLocaleString()}</p>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="p-6 bg-muted/5 space-y-3 border-t border-border">
                  ...
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-bold">{shippingCost > 0 ? `+ ৳${shippingCost}` : "FREE"}</span>
                  </div>
                ...
                <div className="px-6 py-4 bg-muted/20 border-t border-border text-center">
                  <p className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">
                    Estimated Delivery: {form.shipping_area === "inside" ? "1-2 Days" : "3-5 Days"}
                  </p>
                </div>
              </div>
            </div>
